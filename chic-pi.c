...
void master () {
    ...
    while ( /* there are jobs unprocessed */ || /* there are slaves still working on
jobs */ ) {
...
// here we have unprocessed jobs , we send one job to the slave
                job = /* compute job */
                    ...
                    MPI_Send ( msg_buffer ,/*...*/ , slave_rank , TAG_JOB_DATA ,
                               MPI_COMM_WORLD );
...
// We got a result message
            MPI_Recv ( result_data_buffer , /*...*/ , slave_rank , TAG_RESULT ,
